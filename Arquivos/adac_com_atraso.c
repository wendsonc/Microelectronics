...
#define atraso 13000 //Parâmetro alterado até conseguir zero erro no asimut com atraso
...
main(){
  ...
  DEF_GENPAT("adac_com_atraso_13000");
	DECLAR("sel0", ":2", "B", IN, "","");
	DECLAR("sel1", ":2", "B", IN, "","");
	DECLAR("clk", ":2", "B", IN, "","");
...
//	DECLAR("acc", ":2", "B", INOUT, "3 down to 0", "");
  ...
  AFFECT ("0","vss","0b0");
	...
	//			AFFECT (inttostr(cur_vect), "adac", inttostr(adac(a, sel0, sel1)));
				...
				cur_vect = cur_vect + atraso; // incremento para atrasar o sinal de saída
